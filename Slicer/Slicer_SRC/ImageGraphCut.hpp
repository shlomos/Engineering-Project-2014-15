...
unsigned int ImageGraphCut::AddBidirectionalEdge(unsigned int numberOfEdges, const unsigned int source, const unsigned int target, const float weight)
{
...
// TEdges: edges between vertices and source & target
//template <typename TImage, typename TPixelDifferenceFunctor>
void ImageGraphCut::CreateTEdges()
{
	std::cout << "CreateTEdges()" << std::endl;

	int* imageSize = this->_selection->getExtent();

	unsigned int expectedNumberOfTEdges = 2 * 2 * (imageSize[1] * imageSize[3] * imageSize[5]);
	...
	vtkPointData* pointsData = selection_structured_points->GetPointData();
	vtkIntArray* selection_scalars = (vtkIntArray*)pointsData->GetScalars();

	for (int i = 0; i < this->_selection->GetNumberOfPoints(); i++){
		if (selection_scalars[i] == FOREGROUND)
		{
			currentNumberOfEdges = AddBidirectionalEdge(currentNumberOfEdges, i, this->SinkNodeId, 1); // because shlomo dont like 0
			currentNumberOfEdges = AddBidirectionalEdge(currentNumberOfEdges, i, this->SourceNodeId, std::numeric_limits<int>::max());
		}
		else if (selection_scalars[i] == BACKGROUND)
		{
			currentNumberOfEdges = AddBidirectionalEdge(currentNumberOfEdges, i, this->SinkNodeId, std::numeric_limits<int>::max());
...
{
	this->SinkNodeId   = this->_selection->GetNumberOfPoints();
	this->SourceNodeId = this->_selection->GetNumberOfPoints() + 1;
}
